...
const GenerateTextAndTitleFromImagesOutputSchema = z.object({
  ...
  articleContent: z.string().describe('從圖片中完整提取出的所有文字內容，並已重組成一篇通順、流暢的文章。'),
});
...
您的任務分為兩部分：
1.  **文字提取與重組**：請仔細辨識「所有」提供的圖片，將其中包含的「所有文字內容」完整地提取出來。接著，最重要的一步是，根據上下文語意，將因換行而斷裂的句子和段落重新連接，組合成一篇「通順、連貫、流暢、符合邏輯且易於閱讀的完整文章」。請確保內容的完整性和順序的正確性。
2.  **標題生成**：根據您剛剛重組好的完整文章內容，生成一個最能代表文章主旨、簡潔且吸引人的「標題」。

**重要指令：**
-   當提供多張圖片時，請將它們視為一個**連續、完整的文本**來處理。
-   輸出的 \`articleContent\` 必須是經過您編輯重組後，流暢通順的純文字文章。
