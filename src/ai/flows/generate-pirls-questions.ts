...
const PirlsQuestionSchema = z.object({
  ...
  explanation: z
    .string()
    .describe('解題引導，僅提示在文本中的具體段落、句子範圍或區域以尋找解題線索。此說明「絕對不可」揭露或暗示正確答案，也不可解釋選項。需使用台灣常用語彙的繁體中文。'),
  pirlsLevel: z
...
針對「說明」(explanation) 欄位：請以**完全繁體中文**提供解題引導。**此引導「絕對不可」直接或間接暗示哪個選項是正確答案，也不可解釋為何某選項正確或錯誤。** 其「唯一目的」是提示使用者在提供的文本中「哪一個具體段落、句子範圍或特定區域」可以找到与题目相关的資訊或思考線索，從而幫助他們自行推敲答案。說明文字必須使用台灣讀者習慣的自然語氣與詞彙。
