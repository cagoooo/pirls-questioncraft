...
export async function exportPIRLStoPaGamOQuizGroup(
  ...
  try {
    ...
    questionsOutput.questions.forEach((q, index) => {
      ...
      // v1.0 Format Mapping - Corrected column indices
      ...
      // G: 標題多媒體檔名 (null)
      row[7] = articleContent;                        // H: 內容(必填)
      // I: 內容多媒體檔名 (null)
      row[9] = q.question;                            // J: 題目(必填)
      // K: 題目多媒體檔名 (null)
      ...
      // S, T are for Option E and its media (null)
    ...
    // Merged cells for the header - v1.0 does not have these complex merges
    // The visual grouping is done by the empty columns.
