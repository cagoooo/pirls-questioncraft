...
export async function exportPIRLStoPaGamOQuizGroup(
  ...
  try {
    ...
    questionsOutput.questions.forEach((q, index) => {
      ...
      // G: 標題多媒體檔名 (null) - This is incorrect based on template, G should be content. Let's fix all indices.
      
      // Corrected indices based on v1.0 template screenshot
      row[6] = articleContent;                        // G: 內容(必填)
      // H: 內容多媒體檔名 (null)
      ...
      // R, S, T, T are for Option D media, Option E, and Option E media (null)
      row[20] = optionLabels[q.correctAnswerIndex];   // U: 正確答案
      row[21] = q.explanation;                        // V: 文字詳解
      // W, X, Y are for explanation media and option shuffling (null)
